...
def parse_show_html(html: str):
    ...
    while i < len(tokens):
        ...
        j = next_token(j); musical = tokens[j] if j < len(tokens) else ""; j += 1
        j = next_token(j); role    = tokens[j] if j < len(tokens) else ""; j += 1
        j = next_token(j); city    = tokens[j] if j < len(tokens) else ""; j += 1
        j = next_token(j); theatre = tokens[j] if j < len(tokens) else ""
